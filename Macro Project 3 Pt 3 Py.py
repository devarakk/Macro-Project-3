...
def compute_summary_stats(df, company_name):
    return pd.DataFrame({
        ...
        'Cash & Equivalents (cheq)': [df.loc[pre_gfc, 'cheq'].mean(), df.loc[during_gfc, 'cheq'].mean(), df.loc[post_gfc, 'cheq'].mean()],
    })
...
display(summary_df)
summary_df.to_excel("firm_gfc_final.xlsx", index=False)
###############################################################################################################################
...
# plot cash and short term investments 
plt.figure(figsize=(10, 5))
plt.plot(msft_df['date'], msft_df['cheq'], label='MSFT', marker='o', linestyle='-')
plt.plot(wmt_df['date'], wmt_df['cheq'], label='WMT', marker='o', linestyle='-')
plt.plot(ibm_df['date'], ibm_df['cheq'], label='IBM', marker='o', linestyle='-')
...
plt.title('Cash and Short Term Investments Over Time')
plt.legend()
plt.grid(True)
plt.savefig("cash_plot.png", dpi=300, bbox_inches='tight')
plt.show()
